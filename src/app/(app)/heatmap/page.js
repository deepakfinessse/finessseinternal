...
import { listTasks, statusHeatmap } from "@/lib/pm-data";
...
export default async function HeatmapPage({ searchParams }) {
  ...
  const [tasks, people, matrix] = await Promise.all([
    listTasks(user, {}),
    user.can("task:read:all") ? listUsers({ status: "active" }) : [],
